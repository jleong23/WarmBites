  ...
  <main>
      ...
      <div class="container flex flex-col lg:flex-row items-center py-20 gap-10">
        <div class="flex-1 order-2 lg:order-1 text-center lg:text-left">
          <h1 class="text-4xl lg:text-6xl font-extrabold mb-6 text-balance">
            Easiest Dishes To Make!
          </h1>
          <p class="text-xl lg:text-2xl mb-8 text-balance">
            Discover recipes helping you to find the easiest way to cook.
          </p>
          <button
            class=" py-2 text-blue-500 self-start bg-yellow-500 p-5 rounded-md text-lg cursor-pointer"
          >
            Browse Recipes
          </button>
        </div>
        <div class="flex-1 order-1 lg:order-2">
          <NuxtImg sizes="xs:100%vw sm:667vw" format="webp" densities="x1" src="/nuxt-course-hero.png" alt="" />
        </div>
      </div>
    </section>
    <section class="py-20 container">
      ...
      <div v-if="!error" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-8">
        <div v-for="recipe in data?.recipes" class="flex flex-col shadow rounded-md" :key="recipe.id">
          <NuxtImg 
            :src="recipe.image"
          ...
          <div class="flex flex-col py-6 px-4 flex-1">
            ...
            <NuxtLink
              ...
              class="btn px-4 py-2 text-white self-start bg-yellow-700 rounded-md text-base lg:text-lg cursor-pointer">
              View
            </NuxtLink>
          </div>
        </div>
